...
export default function HabitPlanner() {
  ...
  return (
    <section id="habit-planner" className="rounded-2xl bg-white p-4 sm:p-6 shadow-soft border border-indigo-100">
      <h2 className="text-xl sm:text-2xl font-bold text-indigo-800">7-Day Habit Planner</h2>
      ...
      <div className="mt-5 space-y-3">
        {habitTemplates.map((habit) => (
          <label key={habit.id} className="flex gap-3 items-start border border-indigo-100 rounded-xl p-3 sm:p-4 bg-indigo-50/30">
            <input
              type="checkbox"
            ...
            />
            <div>
              <p className="font-semibold text-indigo-900 text-sm sm:text-base">{habit.title} ({habit.minutes} min)</p>
