...
export default function WellbeingCheckIn() {
  ...
  return (
    <section className="rounded-2xl bg-white p-6 shadow-soft border border-indigo-100">
      <h2 className="text-2xl font-bold text-indigo-800">Weekly Resilience Check-In</h2>
      ...
      <div className="mt-6 space-y-6">
        {checkInQuestions.map((question) => (
          <fieldset key={question.id} className="rounded-xl border border-indigo-100 p-4">
            <legend className="font-medium text-indigo-900">{question.prompt}</legend>
      ...
      <div className="mt-6 rounded-xl bg-indigo-50 p-4 border border-indigo-100">
        ...
        {(result.riskLevel === 'high' || result.riskLevel === 'critical') && (
          <a
            ...
            className="inline-block mt-4 px-4 py-2 rounded-lg bg-red-600 text-white font-semibold no-underline hover:bg-red-700"
          >
