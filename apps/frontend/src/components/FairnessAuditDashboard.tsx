      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-amber-500">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4">
            <div className="min-w-0">
              <p className="text-sm text-slate-600">Policy Version</p>
        ...
        <div id="compliance" className="grid grid-cols-1 md:grid-cols-3 gap-4 scroll-mt-24">
          <div className="bg-white rounded-lg shadow p-6 border-t-4 border-blue-500">
            ...
            <p className="text-xs text-slate-600">{parityCompliant} of {metrics.length} cohorts within band</p>
            <p className="text-xs text-slate-500 mt-2">Target: ±{(policy.exposureParityBand * 100).toFixed(0)}% exposure</p>
          </div>

          <div className="bg-white rounded-lg shadow p-6 border-t-4 border-green-500">
            <h3 className="text-sm font-semibold text-slate-600 mb-2">Quality Parity Compliance</h3>
            <p className="text-3xl font-bold text-green-700 mb-1">{qualityComplianceRate.toFixed(1)}%</p>
            <p className="text-xs text-slate-600">{qualityCompliant} of {metrics.length} cohorts above floor</p>
            <p className="text-xs text-slate-500 mt-2">Floor: {(policy.qualityFloor * 100).toFixed(0)}% quality</p>
          </div>

          <div className="bg-white rounded-lg shadow p-6 border-t-4 border-purple-500">
            <h3 className="text-sm font-semibold text-slate-600 mb-2">System Status</h3>
            <p className="text-3xl font-bold text-purple-700 mb-1">{totalMatches}</p>
            <p className="text-xs text-slate-600">matches across {totalCycles} cycles</p>
            <p className="text-xs text-slate-500 mt-2">Fairness-adjusted: {auditLog.filter(e => e.fairnessAdjustment !== 0).length}</p>
          </div>
        </div>
          ...
          <div className="grid gap-3 p-4 md:hidden">
            {metrics.map((m) => {
              ...
              return (
                ...
                >
                  <span className="block text-sm font-semibold text-slate-900">{m.cohort}</span>
                  <span className="mt-3 grid grid-cols-2 gap-3 text-xs text-slate-600">
                    <span>
                      <span className="block font-semibold text-slate-500">Population</span>
                      {(m.populationShare * 100).toFixed(1)}%
                    </span>
                    <span>
                      <span className="block font-semibold text-slate-500">Match Share</span>
                      {(m.matchExposure * 100).toFixed(1)}%
                    </span>
                    <span>
                      <span className="block font-semibold text-slate-500">Parity Gap</span>
                      <span className={parityOk ? 'font-bold text-green-700' : 'font-bold text-red-700'}>
                        {parityOk ? 'Within band' : 'Review'} {(m.exposureParity * 100).toFixed(1)}%
                      </span>
                    </span>
                    <span>
                      <span className="block font-semibold text-slate-500">Matches</span>
                      {m.matchCount}
                    </span>
                  </span>
                </button>
          ...
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-6 py-3 text-left font-semibold text-slate-700">Cohort</th>
                  <th className="px-6 py-3 text-center font-semibold text-slate-700">Pop. Share</th>
                  <th className="px-6 py-3 text-center font-semibold text-slate-700">Exposure</th>
                  <th className="px-6 py-3 text-center font-semibold text-slate-700">Match %</th>
                  <th className="px-6 py-3 text-center font-semibold text-slate-700">Parity Gap</th>
                  <th className="px-6 py-3 text-center font-semibold text-slate-700">Avg Quality</th>
                  <th className="px-6 py-3 text-center font-semibold text-slate-700">Fair Adj.</th>
                  <th className="px-6 py-3 text-center font-semibold text-slate-700">Matches</th>
                </tr>
              </thead>
              <tbody>
                {metrics.map((m) => {
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-3 font-medium text-slate-900">{m.cohort}</td>
                      <td className="px-6 py-3 text-center text-slate-600">{(m.populationShare * 100).toFixed(1)}%</td>
                      <td className="px-6 py-3 text-center text-slate-600">{(m.candidateExposure * 100).toFixed(1)}%</td>
                      <td className="px-6 py-3 text-center text-slate-600">{(m.matchExposure * 100).toFixed(1)}%</td>
                      <td className={`px-6 py-3 text-center font-semibold ${
                        parityOk ? 'text-green-700' : 'text-red-700'
                      }`}>
                        {parityOk ? 'Within band' : 'Review'} {(m.exposureParity * 100).toFixed(1)}%
                      </td>
                      <td className="px-6 py-3 text-center text-slate-600">{(m.averageQuality * 100).toFixed(0)}%</td>
                      ...
                      <td className="px-6 py-3 text-center text-slate-900 font-semibold">{m.matchCount}</td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        <div id="audit-log" className="bg-white rounded-lg shadow overflow-hidden scroll-mt-24">
          <div className="p-6 border-b border-slate-200 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-600">Recent fairness-adjusted matches</p>
            </div>
            <button
                ...
                >
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="text-xs">{entry.cohortA} to {entry.cohortB}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-xs">Status</p>
                      <p className={`font-semibold text-xs ${entry.matchAccepted ? 'text-green-700' : 'text-slate-600'}`}>
                  ...
                  <div className="mt-2 grid grid-cols-4 gap-3 text-xs">
                    <div className="bg-blue-50 p-2 rounded">
                      <p className="text-slate-600">Phase 1</p>
                      <p className="font-mono font-semibold text-blue-700">{entry.phase1Score.toFixed(3)}</p>
                    </div>
                    <div className="bg-green-50 p-2 rounded">
                      <p className="text-slate-600">Phase 2</p>
                      <p className="font-mono font-semibold text-green-700">{entry.phase2Score.toFixed(3)}</p>
                    </div>
                    <div className={`${entry.fairnessAdjustment !== 0 ? 'bg-amber-50' : 'bg-slate-50'} p-2 rounded`}>
                      <p className="text-slate-600">Adjustment</p>
                      <p className={`font-mono font-semibold ${entry.fairnessAdjustment > 0 ? 'text-green-700' : entry.fairnessAdjustment < 0 ? 'text-red-700' : 'text-slate-600'}`}>
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded">
                ...
                <span className="text-lg font-semibold text-slate-900">±{(policy.exposureParityBand * 100).toFixed(0)}%</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded">
                <span className="text-slate-700 font-medium">Under-exposure Boost</span>
                <span className="text-lg font-semibold text-green-700">+{(policy.underExposureBoost * 100).toFixed(0)}%</span>
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded">
                <span className="text-slate-700 font-medium">Over-exposure Penalty</span>
                <span className="text-lg font-semibold text-red-700">−{(policy.overExposurePenalty * 100).toFixed(0)}%</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded">
                <span className="text-slate-700 font-medium">Quality Floor</span>
                <span className="text-lg font-semibold text-slate-900">{(policy.qualityFloor * 100).toFixed(0)}%</span>
              </div>
            </div>
          </div>
        </details>
