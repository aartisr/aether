...
import { suspendRecruitmentPeer } from '../../../../../../lib/peer-recruitment/store';
...
export async function POST(request: NextRequest, context: { params: { peerId: string } }) {
  ...
  try {
    ...
    const peer = await suspendRecruitmentPeer(peerId, body.actorId ?? 'api', body.reason);
    ...
    return NextResponse.json(
      {
        ...
        message: error instanceof Error ? error.message : 'Unknown suspension error.',
      },
      { status: 500 }
    );
  }
